...
/** Categories in the shared order_system database. */
class RefCategory extends MysqlModel
{
    ...
    /** Active categories mapped to the flat shape the public storefront expects. */
    ...
    public function nameExists(string $name, ?int $exceptId = null): bool
    {
        ...
        return (int)$this->run($sql, $params)->fetchColumn() > 0;
    }
    ...
    {
        $row = $this->prepareRow($data);
        unset($row['slug']); // keep stable unless name handling sets it explicitly below
        if (isset($data['slug'])) {
            $row['slug'] = $data['slug'];
        }
    ...
    {
        $cat = $this->run("SELECT image_path FROM {$this->table} WHERE id = ?", [$id])->fetch();
        if ($cat) {
    ...
    /** Slugify a name the same way Laravel's Str::slug does for our purposes. */
