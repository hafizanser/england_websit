...
/**
 * Authoritative cart pricing + offers engine (port of the legacy
 * Services\PricingService). Delivery is always 0 (no delivery charges).
 * Promo codes / auto offers come from the seeded promo set (SeedOffers).
 */
class PricingService
{
    ...
    public function hydrate(array $rows): array
    {
        ...
        $catalog = $this->products->byStorefrontIds($ids);
        $items = [];
        foreach ($rows as $r) {
        ...
        switch ($offer['type']) {
            ...
            case 'percent':
                ...
                if (!empty($cfg['categoryId'])) {
                    $base = 0;
                    foreach ($items as $it) {
                        if ($it['categoryId'] === $cfg['categoryId']) {
                            $base += $it['lineTotal'];
                        }
                    }
                }
    ...
    public function totals(array $items, ?string $appliedCode = null): array
    {
        ...
        $codeStatus = null;
        if ($appliedCode) {
            $offer = SeedOffers::byCode($appliedCode);
